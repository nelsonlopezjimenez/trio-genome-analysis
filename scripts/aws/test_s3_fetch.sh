...
# impostor test, now generalized to all chromosomes via scripts/generate_cds_bed.py
# (data/reference/cds_regions/chr22_cds_regions.bed, 4,186 CDS regions, 0.7 Mb vs.
# chr22's 40.3 Mb) -- see HANDOFF.md's "Data acquisition for population-scale sources"
# section.
#
...
$SCP "$REPO/data/reference/cds_regions/chr22_cds_regions.bed" "ec2-user@$PUBLIC_IP:~/trio-genome/data/"
...
echo "    (Amazon Linux 2023 has no bcftools in default dnf repos, confirmed 2026-09-03 --"
echo "    installing via micromamba/bioconda instead: a single static binary, no compilation,"
echo "    no full-conda-installer wait)"
echo "    (using python3's tarfile module to extract -- this AMI's tar shells out to an"
echo "    external bzip2 binary that isn't installed by default, confirmed on this run)"
$SSH "export MAMBA_ROOT_PREFIX=~/micromamba && mkdir -p ~/bin && \
    curl -Ls https://micro.mamba.pm/api/micromamba/linux-64/latest -o /tmp/micromamba.tar.bz2 && \
    python3 -c \"import tarfile; tarfile.open('/tmp/micromamba.tar.bz2').extract('bin/micromamba', '/home/ec2-user')\" && \
    chmod +x ~/bin/micromamba && \
    ~/bin/micromamba create -y -n bcf -c bioconda -c conda-forge bcftools && \
    ~/bin/micromamba run -n bcf bcftools --version | head -1"

echo "    bcftools installed -- running the actual BED-restricted fetch (sample $SAMPLE)"
$SSH "export MAMBA_ROOT_PREFIX=~/micromamba && cd ~/trio-genome && \
    time ~/bin/micromamba run -n bcf bcftools view -s $SAMPLE -R data/chr22_cds_regions.bed -Oz \
    -o /tmp/${SAMPLE}_chr22_cds.vcf.gz '$VCF_URL' && \
    echo 'variant count:' \$(~/bin/micromamba run -n bcf \
    bcftools view -H /tmp/${SAMPLE}_chr22_cds.vcf.gz | wc -l) && \
